from __future__ import annotations

import io
import os
import tempfile
from pathlib import Path

import pytest
from fastapi.testclient import TestClient
from sqlalchemy.orm import Session

from tests.conftest import auth_headers, create_user


@pytest.fixture(autouse=True)
def temp_upload_dir(monkeypatch):
    """Redirect uploads to a temporary directory during tests."""
    with tempfile.TemporaryDirectory() as tmpdir:
        monkeypatch.setattr("app.core.config.settings.upload_dir", tmpdir)
        yield tmpdir


def test_upload_valid_jpeg(client: TestClient, db: Session):
    user = create_user(db, "uploader@example.com")

    # Minimal 1×1 white JPEG bytes
    jpeg_bytes = (
        b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00"
        b"\xff\xdb\x00C\x00\x08\x06\x06\x07\x06\x05\x08\x07\x07\x07\t\t\x08\n\x0c"
        b"\x14\r\x0c\x0b\x0b\x0c\x19\x12\x13\x0f\x14\x1d\x1a\x1f\x1e\x1d\x1a\x1c"
        b"\x1c $.' \",#\x1c\x1c(7),01444\x1f'9=82<.342\x1edL\t\x04\x05"
        b"\xff\xc0\x00\x0b\x08\x00\x01\x00\x01\x01\x01\x11\x00"
        b"\xff\xc4\x00\x1f\x00\x00\x01\x05\x01\x01\x01\x01\x01\x01\x00\x00\x00\x00"
        b"\x00\x00\x00\x00\x01\x02\x03\x04\x05\x06\x07\x08\t\n\x0b"
        b"\xff\xc4\x00\xb5\x10\x00\x02\x01\x03\x03\x02\x04\x03\x05\x05\x04\x04\x00"
        b"\x00\x01}\x01\x02\x03\x00\x04\x11\x05\x12!1A\x06\x13Qa\x07\"q\x142\x81"
        b"\x91\xa1\x08#B\xb1\xc1\x15R\xd1\xf0$3br\x82\t\n\x16\x17\x18\x19\x1a%&"
        b"'()*456789:CDEFGHIJSTUVWXYZcdefghijstuvwxyz\x83\x84\x85\x86\x87\x88\x89"
        b"\x8a\x92\x93\x94\x95\x96\x97\x98\x99\x9a\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9"
        b"\xaa\xb2\xb3\xb4\xb5\xb6\xb7\xb8\xb9\xba\xc2\xc3\xc4\xc5\xc6\xc7\xc8"
        b"\xc9\xca\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xe1\xe2\xe3\xe4\xe5\xe6"
        b"\xe7\xe8\xe9\xea\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa"
        b"\xff\xda\x00\x08\x01\x01\x00\x00?\x00\xfb\xd4P\x00\x00\x00\x1f\xff\xd9"
    )

    response = client.post(
        "/api/upload/avatar",
        files={"file": ("photo.jpg", io.BytesIO(jpeg_bytes), "image/jpeg")},
        headers=auth_headers(user),
    )
    assert response.status_code == 200
    data = response.json()
    assert "avatar" in data
    assert data["avatar"].startswith("/uploads/")


def test_upload_invalid_content_type(client: TestClient, db: Session):
    user = create_user(db, "uploader2@example.com")
    response = client.post(
        "/api/upload/avatar",
        files={"file": ("script.exe", io.BytesIO(b"MZ\x90\x00"), "application/octet-stream")},
        headers=auth_headers(user),
    )
    assert response.status_code == 400


def test_upload_requires_auth(client: TestClient):
    response = client.post(
        "/api/upload/avatar",
        files={"avatar": ("photo.jpg", io.BytesIO(b"fake"), "image/jpeg")},
    )
    assert response.status_code == 401
